...
public class BlockShuffle {
    ...
    public static void start() {
        if (bukkit_task != null)
            bukkit_task.cancel();
        broadcastMessage("§A§LEt c'est partiiiiiiit pour un block shuffle!");
        gameEnded = false;
        generate();
    }
    ...
    public static void generate() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            player.sendMessage("§C" + "Tu dois te tenir sur le bloc : §L" + material.name().replace("_", " ").toLowerCase() + "§r§C en moins de 5 minutes !");
        }

        bukkit_task = Bukkit.getScheduler().runTaskLaterAsynchronously(FunWorld.instance, BlockShuffle::showLooser, TIME_IN_MINUTES * 60 * 20);
    }
